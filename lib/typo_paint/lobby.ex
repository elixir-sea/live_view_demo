defmodule TypoPaint.Lobby do
  ...
  def init(_init_arg) do
    ...
    {:ok,
     %{
       games: %{id => %{:status => :pending, :game_id => nil, :pos_1 => nil, :pos_2 => nil}},
       players: %{}
     }}
  end
  ...
  def handle_call({:join_game, player_id, lobby_game_id, pos}, _from, lobby) do
    ...
    case lobby.players[player_id].lock do
      ...
      _ ->
        ...
        lobby =
          cond do
            lobby.games[lobby_game_id].pos_1 != nil && lobby.games[lobby_game_id].pos_2 != nil ->
              lobby |> start_game(lobby_game_id)
  ...
  def join_game(player_id, lobby_game_id, pos) do
    GenServer.call(
      __MODULE__,
      {:join_game, player_id, lobby_game_id, String.to_existing_atom(pos)}
    )
  end
